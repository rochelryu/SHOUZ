...
class ExplicationEvent extends StatefulWidget {
  int typeRedirect;//0 if for Navigator.pop, 1 for Navigator.push in choicePackage
  ExplicationEvent({required this.typeRedirect, required Key key});

  @override
  _ExplicationEventState createState() => _ExplicationEventState();
}

class _ExplicationEventState extends State<ExplicationEvent> {
    ...
    return Container(
      child: Scaffold(
        ...
        body: Stack(
          ...
          children: <Widget>[
            PageView.builder(
              itemCount: pageExplicationEventList.length,
              controller: _controller,
              onPageChanged: (index) {
                setState(() {
                  _counter = index;
                });
                if (_counter == pageExplicationEventList.length - 1)
                  lastPage = true;
                else
                  lastPage = false;
              },
                  ...
                  children: <Widget>[
                    AnimatedBuilder(
                      ...
                      builder: (context, child) {
                        var page = pageExplicationEventList[index];
                        var delta;
                        var y = 1.0;
            ...
            Positioned(
              ...
              child: Container(
                width: 180.0,
                child: PageIndicator(_counter, pageExplicationEventList.length),
              ),
            ),
            Positioned(
              right: 30.0,
